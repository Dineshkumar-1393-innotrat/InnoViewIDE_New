...
//         enable_audio: true // Enable screen share audio
//       }
//     },
//     permissions: {
...
//           can_request_produce: !isHost // Allow participants to request screen share permission
//         }
//       },
//       chat: {
//         public: {
...
// /**
//  * Create Dyte preset - Use proper group call presets for video meetings
//  */
// export const createDytePreset = async (presetType = 'participant') => {
//   // Use proper group call presets for video meetings
//   const presetName = presetType === 'host' ? 'group_call_host' : 'group_call_participant';
  
//   console.log('ℹ️ Using group call preset:', presetName);
  
//   // Use group call presets which are better for video meetings
//   let payload;

//   if (presetType === 'participant') {
//     // Exact payload as provided for group_call_participant
//     payload = {
//       name: presetName,
//       config: {
//         view_type: 'GROUP_CALL',
...
//     };
//   } else {
//     const cfg = getPresetConfig('host');
//     payload = {
//       name: cfg.name,
//       config: {
//         view_type: cfg.view_type,
//         max_video_streams: cfg.max_video_streams,
//         max_screenshare_count: cfg.max_screenshare_count,
//         media: cfg.media
//       },
...
//   } catch (error) {
//     if (error.status === 409) {
//       console.warn(`Preset ${presetName} already exists. Treating as success.`);
//       return {
//         name: presetName,
//         status: 'exists'
//       };
...
//       })
//     }).then(handleResponse);
//     console.log(`🧾 Participant token response:`, participantResponse);
...
/**
 * Lightweight helper to fetch a participant token for an existing meeting
 */
export const getParticipantToken = async ({
  ...
  if (!trimmedId) {
    throw new Error('Meeting ID is required to fetch participant token');
  }

  const resolvedPreset =
    presetName || (role === 'host' ? 'group_call_host' : 'group_call_participant');
  const resolvedName = name?.trim?.() || (role === 'host' ? 'Host User' : 'Participant');
  const resolvedClientId =
    clientId || `${role === 'host' ? 'host' : 'participant'}_${Date.now()}`;
  ...
  const participantResponse = await fetch(`${EUREKA_BASE_URL}/get-participant-token`, {
    ...
    body: JSON.stringify({
      ...
      client_specific_id: resolvedClientId,
    }),
  }).then(handleResponse);

  const token = participantResponse?.data?.token;
  if (!token) {
    throw new Error('No token returned from get-participant-token API');
  }
  ...
  return {
    ...
    clientSpecificId: resolvedClientId,
  };
};
...
export const joinExistingMeeting = async (existingMeetingId, participantName, asHost = false) => {
  try {
    console.log(
      `🎫 Joining existing meeting as ${asHost ? 'HOST' : 'participant'}:`,
      existingMeetingId,
    );
