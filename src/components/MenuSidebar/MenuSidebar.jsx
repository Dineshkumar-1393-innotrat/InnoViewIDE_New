...
const MenuSidebar = ({ activeProjectId, onFileSelect, onCreateFile }) => {
  const [isOpen, setIsOpen] = useState(true);
  const [fileTree, setFileTree] = useState([]);
  const [expandedFolders, setExpandedFolders] = useState({});
  const [isLoading, setIsLoading] = useState(false);
  const [isProductDefined, setIsProductDefined] = useState(false);
  ...
  const fetchFileTree = async () => {
    ...
    try {
      const response = await axios.get(
        `https://eureka.innotrat.in/file-tree/${activeProjectId}`
      );
  ...
  // ---------------------------
  // Render Icon for File / Folder
  // ---------------------------
  const getIcon = (item) => {
    if (item.type === "folder")
      return expandedFolders[item.path] ? <FaFolderOpen /> : <FaFolder />;
  ...
  const renderNode = (node, depth = 0) => {
    ...
    return (
      <Box key={node.path} pl={depth * 16} py={1}>
        <Box
          display="flex"
          alignItems="center"
          cursor="pointer"
    ...
    >
      ...
      >
        ...
        <TabPanels>
          ...
          <TabPanel>
            {fileTree.length === 0 ? (
              <Text opacity={0.7}>No files found.</Text>
            ) : (
          ...
          <TabPanel>
            {isLoading ? (
              ...
              <Text color="green.300">Product is fully defined ✓</Text>
            ) : (
              <Text color="red.300">Product not defined.</Text>
            )}
          </TabPanel>
        </TabPanels>
      </Tabs>

      {/* Collapse Section Example */}
      <Box mt={4}>
        <Box
