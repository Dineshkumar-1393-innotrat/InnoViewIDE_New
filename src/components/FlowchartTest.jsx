...
import Vector from '../images/Vector.svg'; // Text Box
import Vector6 from '../images/Vector 6.svg'; // Chevron Arrow
import Vector7 from '../images/Vector 7.svg'; // Off page Connector
import Vector3 from '../images/Vector 3.svg'; // Arrow
import Vector4 from '../images/Vector 4.svg'; // Dotted Arrow
import newlinea from '../images/newlinea.svg'; // Straight Line
import newlineb from '../images/newlineb.svg'; // Left Arrow
import newlinec from '../images/newlinec.svg'; // Right Arrow
import newlined from '../images/newlined.svg'; // Upward Arrow
...
const PALETTE_GROUPS = [
  ...
  //   title: 'Arrows & Lines',
  //   items: [
  //     { type: 'svgStamp', label: 'Text Box', svgSrc: Vector, icon: (<img src={Vector} alt="Text Box" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Chevron Arrow', svgSrc: Vector6, icon: (<img src={Vector6} alt="Chevron Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Off Page Connector', svgSrc: Vector7, icon: (<img src={Vector7} alt="Off Page Connector" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Arrow', svgSrc: Vector3, icon: (<img src={Vector3} alt="Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Dotted Arrow', svgSrc: Vector4, icon: (<img src={Vector4} alt="Dotted Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Straight Line', svgSrc: newlinea, icon: (<img src={newlinea} alt="Straight Line" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Left Arrow', svgSrc: newlineb, icon: (<img src={newlineb} alt="Left Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Right Arrow', svgSrc: newlinec, icon: (<img src={newlinec} alt="Right Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Upward Arrow', svgSrc: newlined, icon: (<img src={newlined} alt="Upward Arrow" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     // Additional arrows/lines
  //     { type: 'svgStamp', label: 'Arrow Down', svgSrc: arrowdown, icon: (<img src={arrowdown} alt="Arrow Down" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Arrow Up', svgSrc: arrowup, icon: (<img src={arrowup} alt="Arrow Up" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Vertical Line', svgSrc: verticalLine, icon: (<img src={verticalLine} alt="Vertical Line" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Line 76', svgSrc: line76, icon: (<img src={line76} alt="Line 76" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //     { type: 'svgStamp', label: 'Line 77', svgSrc: line77, icon: (<img src={line77} alt="Line 77" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />) },
  //   ],
  // },
];
...
function DiagramEditor() {
    ...
    const onKey = (e) => {
      ...
      if (mod && key === 'c') {
        ...
        if (selectedNodes.length) {
          ...
          clipboardRef.current = selectedNodes.map((n) => ({
            ...
            data: n.data ? JSON.parse(JSON.stringify({ ...n.data, editing: false })) : {},
      ...
      if (mod && key === 'v') {
        ...
        if (clipboard && clipboard.length) {
          ...
          const clones = clipboard.map((item, index) => {
            const offsetFactor = 24;
            const offsetX = offsetFactor + index * 8;
            const offsetY = offsetFactor + index * 8;
            return {
              id: getId(),
              type: item.type,
              position: {
              ...
              data: item.data ? { ...item.data, editing: false } : {},
          ...
          setNodes((nds) => {
            ...
            return deselected.concat(clones);
          });
          if (clones.length) {
            setSelected({ kind: 'node', id: clones[clones.length - 1].id });
          }
          scheduleSnapshot();
        }
        return;
      ...
      if (mod && key === 'd') {
        ...
        if (selected?.kind === 'node') {
          setNodes((nds) => {
            const base = nds.find((n) => n.id === selected.id) || nds.find((n) => n.selected);
            if (!base) return nds;
            const clone = {
              ...base,
              id: getId(),
              position: { x: (base.position?.x || 0) + 24, y: (base.position?.y || 0) + 24 },
              selected: true,
            };
      ...
      if (key === 'delete' || key === 'backspace') {
        e.preventDefault();
        setEdges((eds) => {
          const anySel = eds.some((ed) => ed.selected);
          return eds.filter((ed) => (anySel ? !ed.selected : ed.id !== selected?.id));
        });
        setNodes((nds) => {
          const anySel = nds.some((n) => n.selected);
          return nds.filter((n) => (anySel ? !n.selected : n.id !== selected?.id));
        });
        scheduleSnapshot();
        return;
      }
  ...
  }, [undo, redo, selected, setNodes, setEdges, setSelected, scheduleSnapshot]);
